...
public:
	AreaLight(Point3D pos, Color col)
	:
	pos(pos), col_ambient(col), col_diffuse(col), col_specular(col) {}

	AreaLight(Point3D pos, Color ambient, Color diffuse, Color specular)
	:
	pos(pos), col_ambient(ambient), col_diffuse(diffuse), col_specular(specular) {}
	...
	Point3D get_position() const {
		float r = 0.5 * ((double) rand() / (RAND_MAX));
		float theta = 2 * M_PI * ((double) rand() / (RAND_MAX));
		float phi = 2 * M_PI * ((double) rand() / (RAND_MAX));
		return pos + Vector3D(r*cos(theta)*sin(phi),r*sin(theta)*sin(phi),r*cos(phi));
	}
